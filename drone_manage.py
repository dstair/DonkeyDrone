...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if getattr(cfg, 'USE_DRONE_SIM', False):
        ...
        if cfg.DONKEY_GYM:
            gym = DonkeyGymEnv(cfg.DONKEY_SIM_PATH, host=cfg.SIM_HOST,
    ...
    if model_path:
        ...
        if cfg.TRAIN_BEHAVIORS:
            ...
            V.add(bh, outputs=['behavior/state', 'behavior/label',
    ...
    recording_control = ToggleRecording(cfg.AUTO_RECORD_ON_THROTTLE,
                                        cfg.RECORD_DURING_AI)
    V.add(recording_control, inputs=['user/mode', "recording"],
          outputs=["recording"])
    ...
    def run(self, num_records):
        ...
        if self.last_num_rec_print != num_records or self.force_alert:
            ...
            if num_records % self.cfg.REC_COUNT_ALERT == 0 or self.force_alert:
                self.dur_alert = num_records // self.cfg.REC_COUNT_ALERT * self.cfg.REC_COUNT_ALERT_CYC
                self.force_alert = 0
